...
function buildStringSpan(string, text) {
  ...
  for (let i = 0; i < textArray.length; i++) {
    let c = textArray[i];
    if (accents.has(c) && chars.length > 0) chars.push(chars.pop() + c);
    else chars.push(c);
  }
  let numGlyphs = string.glyphs.length + 1; // glyphs are actually separators between glyphs

  if (chars.length == numGlyphs) return text;
  else if (numGlyphs < chars.length)
    return (
      chars.slice(0, numGlyphs).join("") +
      `<span class="error">${chars.slice(numGlyphs).join("")}</span>`
    );
  else
    return (
      text +
      `<span class="error">${missingGlyph.repeat(
...
function fixText(text) {
  for (let i = 0; i < findReplace.length; i++) {
    let findReplacePair = findReplace[i];
    text = text.replaceAll(findReplacePair.find, findReplacePair.replace);
  }
  return text;
}
...
function setText() {
  if (selected) {
    ...
    if (selected.name === "string") {
      let content = fixText(selected.content);
      ...
      for (let i in selected.strings) {
        contents.push(fixText(selected.strings[i].content));
        if (selected.strings[i].content.length > 0) haveContent = true;
      }
      ...
      for (let i = 0; i < selected.textLines.length; i++) {
        ...
        for (let j = 0; j < textLine.strings.length; j++) {
          contents.push(fixText(textLine.strings[j].content));
          if (textLine.strings[j].content.length > 0) haveContent = true;
        }
        let tlContent = haveContent ? contents.join(" ") : "";
        tlContents.push(tlContent);
        if (haveContent) haveTextLineContent = true;
      }
      ...
      for (let k = 0; k < selected.textBlocks.length; k++) {
        ...
        if (haveTextLineContent) haveTextBlockContent = true;
      }
